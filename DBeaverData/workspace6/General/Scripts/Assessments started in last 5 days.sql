select concat(c.id, ' (', c.email, ')'),b.customer_id as cid,
    ...
    from customer_onboarding_status as b 
    inner join customers as c on b.customer_id = c.id
    where to_date(substring(assessment_parameters->>'processDataFromTime',3,11), 'yy/mm/dd') >= current_date - 5
    limit 10