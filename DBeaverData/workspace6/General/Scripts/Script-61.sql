...
CREATE SEQUENCE IF NOT EXISTS public.user_accounts_user_seq 
CREATE TABLE IF NOT EXISTS public.user_accounts (
	user_id int4 DEFAULT nextval('user_accounts_user_seq'::regclass) NOT NULL,
...
from dblink('dbname=dashboard user=postgres password=FNamsdbvU7Yakv' ,'SELECT user_id, email, password, first_name, last_name, company, phone, role_id, title, is_temp_password, last_updated, parent_user_id, global_partner, user_type, account_type, demo_iaas_providers, activation_token, is_activated, token_creation_time, source_ip_from_request, overview_dashboard_user_id, association_with_overview_dashboard, show_terms_checkbox, is_new_account, self_service_step_id, auto_open_self_service_dialog, free_poc_node_count, user_type_mapping_id, allowed_to_create_orders 
 FROM users') 
     AS dashboard_users(
     		user_id int4,
			email varchar(255),
...
insert into user_accounts (user_id, email, password, last_name, title, phone, role_id, is_temp_password, last_updated, user_type, account_type, demo_iaas_providers, activation_token, is_activated, token_creation_time, source_ip_from_request, overview_dashboard_user_id, association_with_overview_dashboard, show_terms_checkbox, is_new_account, self_service_step_id, auto_open_self_service_dialog, free_poc_node_count, user_type_mapping_id, allowed_to_create_orders)
select                          id, email, encpassword, "name", title, phone,  role_id, temp_password, lastupdated, is_primary_account, encpassword, customer_view, vmware_type, show_migrator, "token", show_designer, is_cloudendure_enabled, show_app_cpu_usage, num_of_wrong_password_entered_consecutively, source_ip_from_request, is_dma_enabled, account_status, is_machine_owner, is_migrator_app_view_enabled, show_dma_columns, show_validate, show_migrator_for_preliminary, is_azure_migrate_enabled, build_user_status, terms_accepted_date 
from users
